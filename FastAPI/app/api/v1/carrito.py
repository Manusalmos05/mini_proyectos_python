...
@api_router.get("/", summary="Ver contenido del carrito", response_description="Carrito mostrado correctemente")
def ver_carrito(db:Session= Depends(get_db), user=Depends(get_current_user)):
    carrito=crud_carrito.obtener_carrito(db, user.id)
    return carrito


@api_router.post("/agregar/{producto_id}",summary="agregar producto al carrito", response_description="producto agregado correctemente")
...
@api_router.delete("/eliminar/{item_id}",summary="eliminar carrito", response_description="Carrito eliminado correctemente")
def eliminar_producto(item_id:int, db:Session=Depends(get_db), user=Depends(get_current_user)):
